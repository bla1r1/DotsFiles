...
Item {
    ...
    Item {
        ...
        Image {
            ...
            anchors.fill: parent
            source: !isVideo ? resolvePath(tsource) : ""
            cache: true
            mipmap: true
            fillMode: {
            ...
            function updateVideo() {
                if (isVideo && tsource.toString().length > 0) {
                    backgroundVideo.source = Qt.resolvedUrl(resolvePath(tsource));

                    if (placeholder.length > 0)
                        source = resolvePath(placeholder);
                }
            }
            ...
            onStatusChanged: {
                if (status === Image.Error) {
                    ...
                    var isDefault = s === "backgrounds/default.jpg" || s.endsWith("backgrounds/default.jpg");
                    if (!isDefault && s !== "") {
