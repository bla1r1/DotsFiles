...
cache_is_fresh() {
    ...
    [[ $(( $(date +%s) - $(stat -c %Y "$DISPLAY_CACHE" 2>/dev/null || stat -f %m "$DISPLAY_CACHE") )) -lt "$DETECT_TTL" ]]
}
...
detect_external_displays() {
    ...
    timeout "$DDC_TIMEOUT" ddcutil detect --brief 2>/dev/null \
        | awk '/^Display/ { print $2 }' > "$DISPLAY_CACHE.tmp" || true
...
for_each_external() {
    ...
    while read -r display; do
        ...
        (
            if [[ "$action" == "dim" ]]; then
                current=$(timeout "$DDC_TIMEOUT" ddcutil getvcp 10 --display "$display" --noverify 2>/dev/null \
                    | sed -n 's/.*current value = *\([0-9]\+\).*/\1/p' | head -n1)
                if [[ -n "$current" ]]; then
                    printf '%s\n' "$current" > "$CACHE_DIR/brightness_display_$display"
                fi
                timeout "$DDC_TIMEOUT" ddcutil setvcp 10 10 --display "$display" --noverify >/dev/null 2>&1 || true
            else
                val=$(cat "$CACHE_DIR/brightness_display_$display" 2>/dev/null)
                timeout "$DDC_TIMEOUT" ddcutil setvcp 10 "${val:-100}" --display "$display" --noverify >/dev/null 2>&1 || true
            fi
        ) &
    done < "$DISPLAY_CACHE"
    wait
