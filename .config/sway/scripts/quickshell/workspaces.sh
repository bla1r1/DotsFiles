...
print_workspaces() {
    ...
    spaces=$(timeout 2 swaymsg -t get_workspaces 2>/dev/null)
    active=$(printf '%s\n' "$spaces" | jq -r 'map(select(.focused))[0].num // 1')

    # Failsafe if swaymsg crashes to prevent jq from outputting errors
    ...
    echo "$spaces" | jq --unbuffered --argjson a "$active" --arg end "$SEQ_END" -c '
        ...
        (map( { ((.num // (.name|tonumber? // 0))|tostring): . } ) | add) as $s
        |
        ...
        [range(1; ($end|tonumber) + 1)] | map(
            ...
            (if $i == $a then "active"
             elif ($s[$i|tostring] != null) then "occupied"
             else "empty" end) as $state |

            # Get window title for tooltip (if exists)
            (if $s[$i|tostring] != null then $s[$i|tostring].name else "Empty" end) as $win |
...
# Listen to Sway socket wrapped in an infinite loop
# ============================================================================
while true; do
    swaymsg -m -t subscribe '["workspace","window","output"]' 2>/dev/null | while read -r _; do
        # Sway can emit bursts while windows are moved or resized. Debounce them
        # into one UI update so the bar stays responsive.
