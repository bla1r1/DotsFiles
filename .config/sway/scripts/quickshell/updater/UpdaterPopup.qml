...
Item {
    ...
    property string commitMessage: "Reading repository status..."
    readonly property string scriptDir: Quickshell.env("QS_SCRIPT_DIR") || (Quickshell.env("HOME") + "/.config/sway/scripts")

    Keys.onEscapePressed: {
        Quickshell.execDetached(["bash", window.scriptDir + "/core/qs_manager.sh", "close"]);
        event.accepted = true;
    }
    ...
    Process {
        command: ["bash", "-c", "repo=\"$HOME/GitHub/DotsFiles\"; if [ -d \"$repo/.git\" ]; then git -C \"$repo\" rev-parse --abbrev-ref --symbolic-full-name @{u} 2>/dev/null || echo origin/main; else echo 'GitHub'; fi"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
    ...
    Process {
        command: ["bash", "-c", "repo=\"$HOME/GitHub/DotsFiles\"; if [ -d \"$repo/.git\" ]; then status=$(git -C \"$repo\" status --short 2>/dev/null); if [ -n \"$status\" ]; then printf '%s\\n' \"$status\"; else echo 'Working tree is clean.'; fi; else echo 'Local DotsFiles checkout was not found.'; fi"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Text { 
                        text: updateBtn.fillLevel > 0 ? "HOLDING..." : "OPEN REPO"
                        font.family: "JetBrains Mono"
                        font.weight: Font.Black
