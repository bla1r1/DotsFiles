    ...
    delegate: Component {
	    ...
	    IpcHandler {
            ...
            property string scriptDir: {
                let dir = Quickshell.env("QS_SCRIPT_DIR");
                return dir && dir.length > 0 ? dir : Quickshell.env("HOME") + "/.config/sway/scripts";
            }
            ...
            function applyUpdateState(txt) {
                txt = (txt || "").trim().split("|")[0];
                barWindow.updateAvailable = (txt === "1" || txt === "true" || txt === "available");
            }
            ...
            function applyRecordingState(txt) {
                txt = (txt || "").trim().split("|")[0];
                barWindow.isRecording = (txt === "1" || txt === "true" || txt === "recording");
            }
            ...
            function applyWidgetState(txt) {
                txt = (txt || "").trim().split("|")[0];
                if (barWindow.activeWidget !== txt) barWindow.activeWidget = txt;
            }

            Process {
                id: widgetInitialReader
                command: ["bash", "-c", "cat /tmp/qs_current_widget 2>/dev/null || echo ''"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyWidgetState(this.text)
                }
            }

            Process {
                id: widgetWatcher
                command: ["bash", barWindow.scriptDir + "/core/qs_bus.sh", "listen", "widget"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyWidgetState(this.text)
                }
                onExited: {
                    widgetBusRestart.start();
                }
            }
            ...
            Timer {
                ...
                interval: 20
                repeat: false
                onTriggered: widgetWatcher.running = true
            }
            ...
            Process {
                ...
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyRecordingState(this.text)
                }
            }

            Process {
                id: recWatcher
                command: ["bash", barWindow.scriptDir + "/core/qs_bus.sh", "listen", "recording"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyRecordingState(this.text)
                }
                onExited: recBusRestart.start()
            }
            ...
            Timer {
                ...
                interval: 20
                repeat: false
                onTriggered: recWatcher.running = true
            }

            Process {
                id: updateInitialReader
                command: ["bash", "-c", "if [ -f ~/.cache/qs_update_pending ]; then echo '1'; else echo '0'; fi"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyUpdateState(this.text)
                }
            }

            Process {
                id: updateWatcher
                command: ["bash", barWindow.scriptDir + "/core/qs_bus.sh", "listen", "update"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: barWindow.applyUpdateState(this.text)
                }
                onExited: updateBusRestart.start()
            }
            ...
            Timer {
                ...
                interval: 20
                repeat: false
                onTriggered: updateWatcher.running = true
            }
            ...
            Process {
                id: wsDaemon
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/workspaces.sh"]
                running: true
            }
            ...
            Process {
                ...
                running: true
                command: ["bash", "-c", "bash " + barWindow.scriptDir + "/quickshell/music/music_info.sh | tee /tmp/music_info.json"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process {
                ...
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/kb_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
            ...
            Process { id: kbWaiter; command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/kb_wait.sh"]; onExited: { kbPoller.running = false; kbPoller.running = true; } }

            // --- AUDIO ---
            Process {
                id: audioPoller; running: true
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/audio_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: audioWaiter; command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/audio_wait.sh"]; onExited: { audioPoller.running = false; audioPoller.running = true; } }

            // --- NETWORK ---
            Process {
                id: networkPoller; running: true
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/network_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
        ...
        Process { id: networkWaiter; command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/network_wait.sh"]; onExited: { networkPoller.running = false; networkPoller.running = true; } }

            // --- BLUETOOTH ---
            Process {
                id: btPoller; running: true
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/bt_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: btWaiter; command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/bt_wait.sh"]; onExited: { btPoller.running = false; btPoller.running = true; } }

            // --- BATTERY ---
            Process {
                id: batteryPoller; running: true
                command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/battery_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: batteryWaiter; command: ["bash", "-c", barWindow.scriptDir + "/quickshell/watchers/battery_wait.sh"]; onExited: { batteryPoller.running = false; batteryPoller.running = true; } }


            Process {
                id: weatherPoller
                command: ["bash", "-c", `
                    echo "$(${barWindow.scriptDir}/quickshell/calendar/weather.sh --current-icon)"
                    echo "$(${barWindow.scriptDir}/quickshell/calendar/weather.sh --current-temp)"
                    echo "$(${barWindow.scriptDir}/quickshell/calendar/weather.sh --current-hex)"
                `]
                stdout: StdioCollector {
                    onStreamFinished: {
            ...
            Item {
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle guide"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/launchers/rofi_show.sh drun"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle settings"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    ...
                                    Quickshell.execDetached(["bash", "-c", "rm -f ~/.cache/qs_update_pending; " + barWindow.scriptDir + "/core/qs_bus.sh send update 0; " + barWindow.scriptDir + "/core/qs_manager.sh toggle updater"]);
                                }
                            }
                        }
                    }
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    hoverEnabled: true
                                    anchors.fill: parent
                                    onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh " + wsName])
                                }
                            }
                        }
                    }
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            MouseArea {
                                ...
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle music"])
                ...
                Rectangle {
                    ...
                    MouseArea {
                        ...
                        anchors.fill: parent
                        hoverEnabled: true
                        onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle calendar"])
                    }
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                MouseArea { id: kbMouse; anchors.fill: parent; hoverEnabled: true; onClicked: Quickshell.execDetached(["swaymsg", "input", "type:keyboard", "xkb_switch_layout", "next"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                MouseArea { id: wifiMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle network wifi"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                MouseArea { id: btMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle network bt"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                MouseArea { id: volMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle volume"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                MouseArea { id: batMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/core/qs_manager.sh toggle battery"]) }
                            }
                        }
                    }
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                Quickshell.execDetached(["bash", "-c", barWindow.scriptDir + "/tools/screenshot.sh"]); 
                            }
                        }
                    }                   
                }
