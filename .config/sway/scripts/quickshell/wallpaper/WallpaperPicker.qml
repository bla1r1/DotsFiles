...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        const closeBusCmd = `bash "${escapeBash(window.busScript)}" send close`;
    ...
    property string configuredWallpaperDir: ""
    readonly property string srcDir: {
        if (window.configuredWallpaperDir !== "") return window.configuredWallpaperDir
    ...
    Process {
        ...
        command: ["bash", "-c", "d=$(jq -r '.wallpaperDir // empty' ~/.config/sway/settings.json 2>/dev/null); d=\"${d/#\\~/$HOME}\"; printf '%s' \"$d\""]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
                if (out.length > 0) window.configuredWallpaperDir = out;
            }
        }
    }
